...
export async function recognizePage(canvas:HTMLCanvasElement,worker:Worker){
 ...
 for(let i=0;i<leftLines.length-1;i++){
  const line=leftLines[i];const match=line.text.trim().match(/^(\d{1,2})\s*[:.]?$/);if(!match)continue;
  const weekday=normalize(leftLines[i+1].text).replace(/^TERGA/,'TERCA').replace(/^SEABADO/,'SABADO').match(/^(SEGUNDA|TERCA|QUARTA|QUINTA|SEXTA|SABADO|DOMINGO|LUNES|MARTES|MIERCOLES|JUEVES|VIERNES)/);if(!weekday)continue;
  ...
  markers.push({day,word:weekday[1],y:line.bbox.y0/2});
 }
