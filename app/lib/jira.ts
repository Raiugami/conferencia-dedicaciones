...
const classifyIssue=(value:string):JiraCategory|undefined=>{const issue=normalize(value);return /\b(?:HOLIDAYS|FERIAS|VACACIONES)\b/.test(issue)?'ferias':/\b(?:AUSENCIA|ATESTAD|CERTIFICADO MEDICO)\b/.test(issue)?'atestado':undefined};
export function jiraCategoriesForDay(person:JiraPerson|undefined,day:string):JiraCategory[]{if(!person)return [];const fromIssues=(person.issueDetails??[]).map(issue=>issue.days[day]>0?classifyIssue(issue.name):undefined).filter((category):category is JiraCategory=>Boolean(category));if(fromIssues.length)return Array.from(new Set(fromIssues));return person.justificationCategories?.[day]??(person.justifications?.[day]?[person.justifications[day]]:[]);}
...
export function jiraDifferences(person:Person,jira:JiraPerson|undefined,month:string,holidays:Record<string,string>){
 ...
 const days=person.days.map(day=>{const key=String(day.day);const pdf=dayResult(person,day,month,holidays).cents??0;const jiraHours=jira.days[key]??0;const justified=jira.justificationHours?.[key]??0;const coveredByJustification=pdf+justified===jiraHours;return {day:day.day,pdf,jira:jiraHours,justified,activities:Math.max(0,jiraHours-justified),coveredByJustification,different:pdf!==jiraHours&&!coveredByJustification&&(!dayResult(person,day,month,holidays).exception)}});
